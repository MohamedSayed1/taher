<div class="exam-pass-image">
    @if ($result['passed_exam'] == true)
        <img style="width: 20%;" src="{{ url('front_them/assets/imgs/success-' . App::getLocale() . '.png') }}"
            alt="">
    @else
        <img style="width: 20%;" src="{{ url('front_them/assets/imgs/fail-' . App::getLocale() . '.png') }}"
            alt="">
    @endif
</div>
<hr>
            ...
            @if ($question->answered == 'yes')
                @if ($question->right_answer == 1)
                    <div class="question success" style="cursor: pointer"
                        onclick="getPreviewAnsweredQuestionGuest({{ $exam_id }},{{ $question->id }} , '{{ $question->answered }}' , {{ json_encode($question->selectedAnswers) }}, {{ $question->right_answer == 1 ? 1 : 0 }}, {{ $question->answer_input }})">
