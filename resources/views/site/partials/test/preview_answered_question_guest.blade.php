            ...
            <div id="answer-containers" class="answer-containers cont-ans-31" style="position: relative">
                ...
                @else
                    @foreach ($question->answers as $answer)
                        ...
                        @else
                            <div
                                style="color: #fff;font-size: 1.5em;text-align: center;width: 40px;height: 40px;background-color: rgb(255 0 0 / 43%);border-radius: 50%;border: 2px solid red;position: absolute;top: {{ $answer->top_position }}% !important; left: {{ $answer->left_position }}% !important;">
                                {{ $answer->{'answer_' . App::getLocale()} }}
                            </div>
                        @endif
                    @endforeach
                ...
                @foreach ($question->answers as $answer)
                    ...
                    <p class="{{ $class }}">
                        <input type="checkbox" id="test{{ $answer->id }}" name="checkbox-group"
                            {{ $answered == 'yes' && in_array($answer->id, $selectedAnswers) ? 'checked' : '' }}
                            disabled>
        ...
        <div class="col-lg-12 col-md-12 col-sm-12 images">
            <ul>
                @foreach ($question->answers as $answer)
                    ...
                    @endphp
                    <li class="{{ $class }}">
                        <input name="img" type="checkbox" class="checkbox-group" {{ $answered == 'yes' && in_array($answer->id, $selectedAnswers) ? 'checked' : '' }} id="cb{{ $answer->id }}" disabled/>
...
<div class="row btns-row">
    <button class=" btn prev" onclick="getprevExamFinish({{ $question->id }},{{ $exam_id }})">
        @if (App::getLocale() == 'ar')
    ...
    <button class=" btn next" onclick="getnextExamFinish({{ $question->id }},{{ $exam_id }})">
